...
引数なしで起動すると、**マシンスペック（RAM・GPU）から最適なモデルを自動選択**して設定ファイルに保存します。
...
CPU のみの場合は `qwen3.5:0.8b-q4_k_m`（4 GB 以上）/ `bonsai-8b`（4 GB 未満）を選択します。
...
| `--llama-model` | `LLAMA_MODEL` | *(必須)* | LLM モデル名またはファイルパス |
...
| `calm3:22b-q4_k_m` | ≈ 13 GB | Apache 2.0 | 日英特化 (CyberAgent)、要 16 GB VRAM |
...
>   CPU/低 VRAM（< 4 GB）環境向けの 1-bit 圧縮モデル（~1.15 GB）として有用。
