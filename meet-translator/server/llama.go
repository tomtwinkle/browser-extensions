...
func (s *server) translateInternal(text, sourceLang, targetLang string, opts ModelOptions, history []contextEntry) (string, error) {
	...
	prompt := buildTranslationPrompt(text, sourceLang, targetLang, template, opts, history)
	s.logVerbose("translate input: %q (model=%s, template=%s, thinking=%v, history=%d)",
		text, s.loadedModelSpec, template, opts.Thinking, len(history))
