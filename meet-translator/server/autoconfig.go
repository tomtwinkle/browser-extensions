...
//	   4  bonsai-8b                 0.454   7168ms    0.288  ← thinking modeで10sタイムアウト多発
//	   5  gemma4:e4b-q4_k_m        0.256  11707ms    0.163  ← 翻訳タスク不適
//
// ※ bonsai-8b は 1-bit 圧縮 (~1.15GB) で CPU/低 VRAM 向けフォールバック。
//    GPU benchmark では thinking mode による長いレスポンスがタイムアウトを引き起こすが、
//    CPU 環境では唯一 4GB RAM 以下で動作する 8B 相当モデルとして有用。
//
// ベンチマークの実行方法:
//
