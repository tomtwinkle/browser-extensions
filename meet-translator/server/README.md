...
| Python 3.10+ (optional) | SenseVoice / WhisperX バックエンド | https://www.python.org/downloads/ |
...
| `bonsai-8b`           | ≈ 1.15 GB | 最初の step-up、1-bit 8B、Thinking 対応、PrismML 必須 |
...
> `bonsai-8b` を使うには `server-prism` が必要です。`make` なら自動で両方ビルドされます。
